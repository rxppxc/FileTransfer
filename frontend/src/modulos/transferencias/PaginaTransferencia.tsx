...
// ── Tipo de archivo ───────────────────────────────────────
type CategoriaArchivo = "imagen" | "pdf" | "word" | "excel" | "ppt" | "zip" | "video" | "audio" | "texto" | "otro";

interface InfoTipo { etiqueta: string; categoria: CategoriaArchivo; }

function infoTipo(mime: string | null, nombre: string): InfoTipo {
  const m  = mime?.toLowerCase() ?? "";
  const ex = nombre.split(".").pop()?.toLowerCase() ?? "";

  if (m.startsWith("image/") || ["jpg","jpeg","png","gif","webp","bmp","svg","tiff"].includes(ex))
